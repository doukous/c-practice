...
void insert(int *num_parts, struct part *inventory) {
  ...
  int insertion_index;
  for (insertion_index = 0; insertion_index < *num_parts; insertion_index++)
      if (inventory[insertion_index].number > part_number)
          break;

  for (int i = *num_parts; i > insertion_index; i--)
      inventory[i] = inventory[i - 1];

  inventory[insertion_index].number = part_number;

  printf("Enter part name: ");
  read_line(inventory[insertion_index].name, NAME_LEN);
  printf("Enter quantity on hand: ");
  scanf("%d", &inventory[insertion_index].on_hand);
  (*num_parts)++;
}
/**********************************************************
*
